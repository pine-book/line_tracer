...
def main():
    ...
    GPIO.setup(REF_L, GPIO.OUT)
    GPIO.setup(REF_R, GPIO.OUT)
    ref_l = GPIO.PWM(REF_L,50)
    ref_r = GPIO.PWM(REF_R,50)
    try:
        while True:
            ...
            #print(input_data)
            interpreter.set_tensor( input_details[0]['index'] , (input_data- input_mean)/input_std)
            interpreter.invoke()
            GPIO.output(LEFT, 0)
            GPIO.output(RIGHT, 0)
            ...
            if output_data[0][0] > 0.9:
                direction = 'l'
                #output_data[0][1] =0

            elif output_data[0][1] > 0.9:
                direction = 'r'
                #output_data[0][0] =0
            else:
                direction = 's'
            print(direction)
            ref_l.start(output_data[0][0]*60 + 3)
            ref_r.start(output_data[0][1]*60 + 3)
            GPIO.output(LEFT, 1)
            GPIO.output(RIGHT, 1)
            #time.sleep(0.1)
